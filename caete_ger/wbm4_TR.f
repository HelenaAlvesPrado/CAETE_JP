...
c     -------------------------O U T P U T S---------------------------
      ...
      real photo_pft(nx,ny,12,q) !Monthly photosynthesis   (kgC m-2 yr)
      real aresp_pft(nx,ny,12,q) !Monthly autotrophic res  (kgC m-2)
      real npp_pft  (nx,ny,12,q) !Monthly net primary produ (kgC m-2)
      
      real lai_pft  (nx,ny,12,q) !Monthly leaf area index (m-2 m-2)
      real clit_pft (nx,ny,12,q) !Monthly litter carbon (kgC m-2)
      real csoil_pft(nx,ny,12,q) !Monthly soil carbon (kgC m-2)
      real hresp_pft(nx,ny,12,q) !Monthly het resp  (kgC/m2)
      real rcm_pft  (nx,ny,12,q) !canopy resistance (s/m) 
...
c     NOVOS OUTPUTS DA BIANCA
      real rml_pft  (nx,ny,12,q) ! Maintenance respiration_leaf (kgC m-2 yr) 
      real rmf_pft  (nx,ny,12,q) ! Maintenance respiration_fineroots (kgC m-2 yr) 
      real rms_pft  (nx,ny,12,q) ! Maintenance respiration_sapwood (kgC m-2 yr) 
      real rm_pft   (nx,ny,12,q) ! Maintenance respiration (kgC m-2 yr) 
      
      real rgl_pft  (nx,ny,12,q) ! Growth respiration_leaf (kgC m-2 yr)
      real rgf_pft  (nx,ny,12,q) ! Growth respiration_fineroots (kgC m-2 yr)
      real rgs_pft  (nx,ny,12,q) ! Growth respiration_sapwood (kgC m-2 yr)
      real rg_pft   (nx,ny,12,q) ! Growth respiration (kgC m-2 yr)
      ...
      real betal(nx,ny,12,q)     ! acumular o beta_leaf (medida de crescimento)
      real betaw(nx,ny,12,q)     ! acumular o beta_wood
      real betaf(nx,ny,12,q)     ! acumular o beta_fineroots
...
c     ------------------------- internal variables---------------------
      ...
      real cleaf1_pft (q)     !conferir se é do dia anterior
      real cawood1_pft(q)
      real cfroot1_pft(q)
...
!     -------------------     
      do i=1,nx
         do j=1,ny
            do p = 1,q   
               cleaf1_pft (p) =  cleaf_ini(i,j,p)     ! Only for Jan 01 (it comes from the spinup)
               cawood1_pft(p) = cawood_ini(i,j,p)     ! Only for Jan 01 (it comes from the spinup)
               cfroot1_pft(p) = cfroot_ini(i,j,p)     ! Only for Jan 01 (it comes from the spinup)
...
!     ====================
     ...
     &             ,betafmes)
               ...
               do p=1,q
                  ...
                  cleaf1_pft(p)  = cleafmes(p)  !It turns the carbon content of the last day of the previous month into the first day of the next month 
                  cawood1_pft(p) = cawoodmes(p) !It turns the carbon content of the last day of the previous month into the first day of the next month
                  cfroot1_pft(p) = cfrootmes(p) !It turns the carbon content of the last day of the previous month into the first day of the next month
...
!        
      ...
      real cl1_pft(npft)        !Initial BIOMASS cleaf compartment (kgC m-2)
      real cf1_pft(npft)        !Initial BIOMASS froot compartment (kgC m-2)
      real ca1_pft(npft)        !Initial BIOMASS froot compartment (kgC m-2)
...
!     ----------------------------OUTPUTS------------------------------
      ...
      real cawoodavg_pft(npft) !Sum of carbon content of a month 
      real cfrootavg_pft(npft) !Sum of carbon roots content of a month
      real ocpavg(npft), betalavg(npft), betawavg(npft), betafavg(npft) !crescimento/decrescimo de um dia pro outro
...
!     RELATED WITH GRIDCELL OCUPATION
      ...
      real OCP_COEFFS(NPFT), ocp_mm(npft) !ocp_mm: used in wbm
...
!     WBM COMMUNICATION (water balance)
      ...
      real ds  (npft)           !Related to snow budget
      real dw  (npft)           !Related to penman subroutine

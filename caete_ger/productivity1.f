...
!
     ...
     &    wmax, ca,ipar,rh,cl1, ca1, cf1, beta_leaf, beta_awood, beta_froot
...
!     
      ...
      real cl1, cf1, ca1        !carbon_leaf,carbon_fineroots,aboveground_wood_biomass
      real beta_leaf, beta_awood, beta_froot    !O que foi adicionado/diminuido de biomassa em relacao ao dia anterior
      LOGICAL LIGTH_LIMIT       !Parameter to identify the PLS with the greatest aboveground wood biomass 
!
!     Output
!     ------
!     
      ...
      real f5                   !Water stress 
      real rm, rml, rmf, rms, rg, rgl, rgf, rgs !Maintenance respiration (leaf,fineroots,sapwood)/growth respiration...
!      
!     Internal
!     --------
!     
...
!
      real tleaf(7)             !leaf turnover time (yr) - used to calculate SLA
      real p21(7)               !vcmax
      double precision sla      !specific leaf area (m2/kg)      
!!!      double precision cl2   !leaf compartment's carbon content (kgC/m2) !!!!devemos adicionar?
      double precision csa      !sapwood compartment´s carbon content (5% of woody tissues(Pavlick,2013)) (kgC/m2)
      ...
      double precision csai     !It calculates sapwood growth (daily) - deveria ser um beta_sapwood?
...
!     ---------------------------------------------------------------
!     
      jc = vm*((ci-mgama)/(ci+(f2*(1.+(p3/f3)))))                       !adicionar f10 pra ver se eh bug
...
!
      ph64 = 0.012*31557600.0*f1*f4sun*f4shade !nao precisa ser multiplicada pela ocp?
...
C      call critical_value2(csa) !conteudo de carbono aboveground
c      
      ncl = 0.034               !nitrogen:carbon content_leaf(gN/gC)
      ncf = 0.034               !nitrogen:carbon content_fineroots(gN/gC)
      ncs = 0.003               !nitrogen:carbon content_sapwood(gN/gC)
c      
c      
      rml64 = (ncl * cl1) * 27. * exp(0.03*temp)     !tentar retomar esse valor de sensibilidade
...
c23456
     ...
     &    OCP_WOOD)
c     esta subrotina calcula a area de cada pft a partir do conteudo 
c     de carbono nos compartimentos vegetais no dia anterior 
c
...
!      
      ...
      do p = 1,npft
         ...
         OCP_COEFFS(P) = 0.0
         OCP_WOOD(P) = .FALSE.            !não entendemos
      enddo
!      
      DO P = 1,NPFT
...
!     GRID CELL OCCUPATION COEFFICIENTS
      IF(TOTAL_BIOMASS .GT. 0.0) THEN
         DO P = 1,NPFT   
            OCP_COEFFS(P) = TOTAL_BIOMASS_PFT(P) / TOTAL_BIOMASS  !OCP_COEFFS = %
            IF(OCP_COEFFS(P) .LT. 0.0) OCP_COEFFS(P) = 0.0
            CALL CRITICAL_VALUE(OCP_COEFFS(P))        !!!!Nao entendemos tb
...
!     GRIDCELL PFT LIGTH LIMITATION BY WOOD CONTENT 
      IF(TOTAL_WOOD .GT. 0.0) THEN
         MAX_INDEX = MAXLOC(TOTAL_W_PFT) !biomassa total de tecidos lenhosos de cada pft (total..)
         I = MAX_INDEX(1)
         OCP_WOOD(I) = .TRUE.
      ENDIF
...
!     
      ...
      delta_e = es*(1. - ur)    !mb
!     
      if ((delta_e.ge.(1./h5)-0.5).or.(rc2.ge.4500)) evap = 0.    !!! houston, we have a problem
...
!     variables
      ...
      real aleaf(npfts)         !npp percentage allocated to leaf compartment
      real aawood(npfts)        !npp percentage allocated to wood compartment
      real afroot(npfts)        !npp percentage allocated to root compartment    
      real tleaf(npfts)         !turnover time/leafs (yr)
      real tawood(npfts)        !turnover time/wood (yr)
      real tfroot(npfts)        !turnover time/fineroots (yr)     

      call pft_id(3, aleaf)
      call pft_id(4, aawood)
      call pft_id(5, afroot)
      call pft_id(6, tleaf)
      call pft_id(7, tawood)
      call pft_id(8, tfroot)
!    

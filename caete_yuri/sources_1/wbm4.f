...
!
     ...
     &  write(*,*) 'working:',i
!
! initialize variables (NO_DATA is undef)
!
      tmin(i,j)   = NO_DATA
      seanpp(i,j) = NO_DATA
      meanpp(i,j) = NO_DATA
      ...
      ave_rc(i,j) = NO_DATA
      ave_runom(i,j) = NO_DATA                                             !calculates annual average canopy resistance
      ave_gsoil(i,j) = NO_DATA
      ave_ssoil(i,j) = NO_DATA
      ave_snowm(i,j) = NO_DATA
!
      do k=1,12
!
      ...
      snowm(i,j,k) = NO_DATA                                            !average snowmelt (mm/day) mm/day == kg m-2 day-1
...
!
      do k=1,12
      wg0(i,j,k) = -1.0
      spre = p0(i,j,k) * 0.01 ! surface pressure (mb)==hPa
      enddo

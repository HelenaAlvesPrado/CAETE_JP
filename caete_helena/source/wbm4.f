...
!     temperature, the WBM calculates the
...
!     
     ...
     $                 ,k)
!     
                  if (npp1(i,j,k).lt.nppmin(i,j)) nppmin(i,j) = npp1(i,j
     $                 ,k)
                  if (npp1(i,j,k).gt.nppmax(i,j)) nppmax(i,j) = npp1(i,j
     $                 ,k)
!     
                  if (npp2(i,j,k).lt.nppmin(i,j)) nppmin(i,j) = npp2(i,j
     $                 ,k)
                  if (npp2(i,j,k).gt.nppmax(i,j)) nppmax(i,j) = npp2(i,j
     $                 ,k)
!     
                  if (npp3(i,j,k).lt.nppmin(i,j)) nppmin(i,j) = npp3(i,j
     $                 ,k)
                  if (npp3(i,j,k).gt.nppmax(i,j)) nppmax(i,j) = npp3(i,j
     $                 ,k)
!     
               enddo
!     
               npptotal(i,j) = meanpp1(i,j) + meanpp2(i,j) + meanpp3(i
     $              ,j)
!     
               mnpp1_f(i,j) = ((meanpp1(i,j))/(npptotal(i,j)))
               if (npptotal(i,j).eq.0.and.meanpp1(i,j).eq.0) mnpp1_f(i
     $              ,j) = 0
!     
               mnpp2_f(i,j) = ((meanpp2(i,j)/npptotal(i,j)))
               if (npptotal(i,j).eq.0.and.meanpp2(i,j).eq.0) mnpp2_f(i
     $              ,j) = 0
!     
               mnpp3_f(i,j) = ((meanpp3(i,j)/npptotal(i,j)))
               if (npptotal(i,j).eq.0.and.meanpp3(i,j).eq.0) mnpp3_f(i
     $              ,j) = 0
!     
...
!     
               if (meanpp1(i,j).gt.0.0) then
                  seanpp(i,j) = (nppmax(i,j) - nppmin(i,j))/(meanpp1(i
     $                 ,j))
               else
                  seanpp(i,j) = (nppmax(i,j) - nppmin(i,j))
               endif
!     
               if (meanpp2(i,j).gt.0.0) then
                  seanpp(i,j) = (nppmax(i,j) - nppmin(i,j))/(meanpp2(i
     $                 ,j))
               else
                  seanpp(i,j) = (nppmax(i,j) - nppmin(i,j))
               endif
!     
               if (meanpp3(i,j).gt.0.0) then
                  seanpp(i,j) = (nppmax(i,j) - nppmin(i,j))/(meanpp3(i
     $                 ,j))
               else
                  seanpp(i,j) = (nppmax(i,j) - nppmin(i,j))
               endif

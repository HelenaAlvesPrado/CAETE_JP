...
!     
      integer i,j,k,p
      integer, parameter :: nx=120,ny=160,q=7         ! q = number of pfts
...
!     
      ...
      real rhaux(nx,ny,12)      !Relative humidity auxiliar
      real, dimension(nx,ny,q) :: cleafin = no_data   !carbon_leaf_initial (spinup output)
     $                          ,cawoodin = no_data   !carbon_wood_initial (spinup output)
     $                          ,cfrootin = no_data   !carbon_fineroots_initial (spinup output)
!     
!     Outputs
!     -------
...
C      WARNING - NEW VARIABLES ---
      ...
      real betal(nx,ny,12,q)     !Biomass increase on leaf compartment
      real, dimension(nx,ny,12):: bl1,bl2,bl3,bl4,bl5,bl6,bl7 !Carbon allocated to growth (monthly sums) 
      real betaw(nx,ny,12,q)     !Biomass increase on wood compartment
      real, dimension(nx,ny,12):: bw1,bw2,bw3,bw4,bw5,bw6,bw7
      real betaf(nx,ny,12,q)     !Biomass increase on fineroots compartment
...
!      
!     dt1 = g1                            !verificar se os valores estao correspontes
!     dt2 = p21 
!     dt3 = aleaf
!     dt4 = aawood
!     dt5 = afroot
...
!     PFT         1       2       3       4       5       6       7 
      ...
      data dt4 /0.28,   0.30,   0.27,   0.12,   0.0,    0.40,   0.14/
      data dt5 /0.42,   0.32,   0.37,   0.43,   0.45,   0.25,   0.31/
      data dt6 /3.0,    2.0,    1.0,    2.0,    1.0,    1.0,    2.0 /
      data dt7 /35.0,   30.0,   25.0,   2.0,    0.0,    32.0,   2.5/
      data dt8 /4.0,    3.5,    3.8,    1.5,    1.0,    3.8,    2.2/ 
     
      if(id .eq. 1 ) then      ! g1
         dt(:) = dt1(:)
      else if(id .eq. 2) then  ! p21
         dt(:) = dt2(:)
      else if(id .eq. 3) then  ! aleaf
         dt(:) = dt3(:)
      else if(id .eq. 4) then  ! awood
         dt(:) = dt4(:)
      else if(id .eq. 5) then  ! afroot
         dt(:) = dt5(:)
      else if(id .eq. 6) then  ! tleaf
         dt(:) = dt6(:)
      else if(id .eq. 7) then  ! tawood
         dt(:) = dt7(:)
      else if(id .eq. 8) then  ! tfroot
         dt(:) = dt8(:)
      else
...
!
      implicit none
!
      integer, parameter :: nt=10000       !number of runs

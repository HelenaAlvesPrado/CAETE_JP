...
! c
    ...
    real ca
      ! Dummy variables
    real, dimension(nx,ny,nt):: pr, t, par, ps_aux,&
                    prec, temp, ipar, p0, wsoil2,&
                    evaptr, rc

    real, dimension(nx,ny) :: lsmk, tmin, seanpp, meanpp, meanhr,&
                              meancs, mphoto, maresp, ave_wsoil,ave_evap,&
                              ave_rc

! c water cycle
    real, dimension(nx,ny) :: waux,waux2,waux3,waux4,waux5,waux6

!c carbon cycle
    real, dimension(nx,ny,nt) :: mp, mar, mnpp
! c
! c Open files
! c ----------
! c
    open( 9,file='../inputs/ps.bin',status='old',form='unformatted',access='direct',recl=4*nx*ny)
    open(10,file='../inputs/lmsk.bin',status='old',form='unformatted',access='direct',recl=4*nx*ny)
    open(11,file='../inputs/pr.bin',status='old', form='unformatted',access='direct',recl=4*nx*ny)
    open(12,file='../inputs/tas.bin',status='old', form='unformatted',access='direct',recl=4*nx*ny)
    open(13,file='../inputs/ipar.bin',status='old',form='unformatted',access='direct',recl=4*nx*ny)
...
! c
    call wbm (prec,temp,lsmk,p0,ca,par,&
              tmin,meanpp,seanpp,mphoto,maresp,meanhr,&
              meancs,wsoil2,evaptr,mnpp,mp,mar,rc,ave_wsoil,ave_evap,ave_rc)
! c

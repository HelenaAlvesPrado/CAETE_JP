subroutine wbm (prec,temp,p0,ca,par, tsoil, wsoil, wsoil2, evapm, runom, rcm)
!c
!c=======================================================================
!c
...
!c 01Dez2016, JPDF: CAETE.__init__()
!c=======================================================================

  !NO MORE ARRAYS <- WBM now works on a basis of only one gridcell


  ! NEW DECLARATION
  real, intent(in) :: prec, temp, p0, ca, par ! jp- change prec to pr and par to rsds__future__


  ! PARAMETERS OF SOIL TEMPERATURE
  ...
  real, parameter :: AUXS = -100                     ! auxiliar variable ? 

  ! OTHER PARAMETERS

  real, parameter :: NO_DATA = -9999.000
  
  ! OUTPUT VARIABLES
  !real, intent(out) :: tmin
  !real, intent(out) :: meanpp
  !real, intent(out) :: seanpp
  !real, intent(out) :: mphoto
  !real, intent(out) :: maresp
  real, intent(out) :: tsoil
  real, intent(out) :: wsoil
  real, intent(out) :: wsoil2
  real, intent(out) :: evapm
  real, intent(out) :: runom
  !real, intent(out) :: photo
  !real, intent(out) :: aresp
  real, intent(out) :: rcm
  !real, intent(out) :: ave_wsoil
  !real, intent(out) :: ave_evap
  !real, intent(out) :: ave_rc

  ! INTERNAL VARIABLES
  real :: t0, t1, gsoil, ssoil, snowm
  real :: emaxm, wg0
  real :: wini, gini, sini
  
  !ANCIENT VARIABLE DECLARATIONS
  !c i/o variables
 ...
 ! NEW SOIL INITIALIZATION
  ...
  ! PREVIOUS WATER BUDGET INITIALIZATION
  !c Water budget
  !c ------------
  !c
  !c for all grid points
  ...
  ! NEW WATER BUDGET INITIALIZATION
  
  wsoil = NO_DATA !soil moisture (mm) or kg m-2
  gsoil = NO_DATA !soil ice (mm)
  ssoil = NO_DATA !soil snow (mm)
  snowm = NO_DATA !average snowmelt (mm/day)
  runom = NO_DATA !average runoff (mm/day)
  evapm = NO_DATA !average actual evapotranspiration (mm/day)
  emaxm = NO_DATA !average maximum evapotranspiration (mm/day)
  wg0   = NO_DATA !soil moisture of the previous year (mm)

  ! NEW WATER BUDGET CALCULATION

  wini = 0.01
  gini = 0.0
  sini = 0.0
  !PREVIOUS WATER BUDGET CALCULATION
!c only for land grid points
!      if (int(lsmk(i,j)).ne.0) then
!c

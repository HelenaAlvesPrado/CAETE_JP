...
contains
  ...
  subroutine readx(nunit,var,x)
    ...
    real(kind=i4),intent(inout) :: var(nx,ny,x)
    real(kind=i4) :: aux(nx,ny)
  ...
  subroutine save_file12(nunit, var)
    ...
    real(kind=i4),intent(in) :: var(nx,ny,12)
    real(kind=i4) :: waux(nx,ny)
    integer(kind=i4) :: i,j,k
    do k=1,nt
       do i=1,nx
          do j=1,ny
  ...
  subroutine savex(nunit, var, x)
    integer(kind=i4), intent(in) ::  nunit,x
    real(kind=i4),intent(in) :: var(nx,ny,x)
    real(kind=i4) :: aux(nx,ny)
    integer(kind=i4) :: i,j,k
    do k=1,x
       do i=1,nx

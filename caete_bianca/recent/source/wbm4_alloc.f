...
c     i/o variables
     ...
     &     nppavg_pft(npls),
     &     mean_photo_tbe(nx,ny),mean_photo_tbd(nx,ny),
     &     mean_photo_herb(nx,ny),mean_photo_pft(nx,ny,npls),
     &     photo_pft(nx,ny,12,npls),photoavg_pft(npls),
     &     mean_ar_tbe(nx,ny),mean_ar_tbd(nx,ny),
     &     mean_ar_herb(nx,ny),mean_ar_pft(nx,ny,npls),
...
c     carbon allocation
     ...
     $     ,ny),cotherini_herb(nx,ny),crepini_herb(nx,ny)
     $     ,cleafini_pft(nx,ny,npls),cleaf1_pft(npls),cawoodini_pft(nx
     $     ,ny,npls),cawood1_pft(npls),cfrootini_pft(nx,ny,npls)
     $     ,cfroot1_pft(npls),mean_cleaf(nx,ny),mean_cawood(nx,ny)
     $     ,mean_cbwood(nx,ny),mean_cfroot(nx,ny),mean_csto(nx,ny)
     $     ,mean_crep(nx,ny),mean_cother(nx,ny),mean_cleaf_pft(nx,ny
     $     ,npls),mean_cleaf_tbe(nx,ny),mean_cleaf_tbd(nx,ny)
     $     ,mean_cleaf_herb(nx,ny),cleaf_pft(nx,ny,12,npls)
     $     ,cl2_pft(npls),cleafavg_pft(npls),cleafi_pft(npls)
     $     ,cleaff_pft(npls),mean_cawood_pft(nx,ny,npls)
     $     ,mean_cawood_tbe(nx,ny),mean_cawood_tbd(nx,ny)
     $     ,mean_cawood_herb(nx,ny),cawood_pft(nx,ny,12,npls)
     $     ,ca2_pft(npls),cawoodavg_pft(npls),cawoodi_pft(npls)
     $     ,cawoodf_pft(npls),mean_cbwood_pft(nx,ny,npls)
     $     ,mean_cbwood_tbe(nx,ny),mean_cbwood_tbd(nx,ny)
     $     ,mean_cbwood_herb(nx,ny),cbwood_pft(nx,ny,12,npls)
     $     ,cb2_pft(npls),cbwoodavg_pft(npls),cbwoodi_pft(npls)
     $     ,cbwoodf_pft(npls),mean_cfroot_pft(nx,ny,npls)
     $     ,mean_cfroot_tbe(nx,ny),mean_cfroot_tbd(nx,ny)
     $     ,mean_cfroot_herb(nx,ny),cfroot_pft(nx,ny,12,npls)
     $     ,cf2_pft(npls),cfrootavg_pft(npls),cfrooti_pft(npls)
     $     ,cfrootf_pft(npls),mean_csto_pft(nx,ny,npls),mean_csto_tbe(nx
     $     ,ny),mean_csto_tbd(nx,ny),mean_csto_herb(nx,ny),csto_pft(nx
     $     ,ny,12,npls),cs2_pft(npls),cstoavg_pft(npls),cstoi_pft(npls)
     $     ,cstof_pft(npls),mean_crep_pft(nx,ny,npls),mean_crep_tbe(nx
     $     ,ny),mean_crep_tbd(nx,ny),mean_crep_herb(nx,ny),crep_pft(nx
     $     ,ny,12,npls),cr2_pft(npls),crepavg_pft(npls),crepi_pft(npls)
     $     ,crepf_pft(npls),mean_cother_pft(nx,ny,npls)
     $     ,mean_cother_tbe(nx,ny),mean_cother_tbd(nx,ny)
     $     ,mean_cother_herb(nx,ny),cother_pft(nx,ny,12,npls)
     $     ,co2_pft(npls),cotheravg_pft(npls),cotheri_pft(npls)
     $     ,cotherf_pft(npls),ctotal_tbe(nx,ny),ctotal_tbd(nx,ny)
     $     ,ctotal_herb(nx,ny),ctotal_gd(nx,ny),ctotal_pft(nx,ny,npls)
     $     ,ocp_tbe(nx,ny),ocp_tbd(nx,ny),ocp_herb(nx,ny),ocp_pft(nx,ny
     $     ,npls)
...
!     mean_cbwood_tbe(i,j)=-999.99 !mean annual belowground biomass for
!     the TBE PFT (kgC/m2)
!     mean_csto_tbe(i,j)=-999.99 !mean annual leaf biomass for the TBE
!     PFT
!     (kgC/m2)
!     mean_crep_tbe(i,j)=-999.99 !mean annual aboveground biomass for
!     the TBE
!     PFT (kgC/m2)
!     mean_cother_tbe(i,j)=-999.99 !mean annual belowground biomass for
!     the
!     TBE PFT (kgC/m2)
...
!     mean_cbwood_tbd(i,j)=-999.99 !mean annual belowground biomass for
!     the TBD PFT (kgC/m2)
!     mean_csto_tbd(i,j)=-999.99 !mean annual aboveground biomass for
!     the TBD
!     PFT (kgC/m2)
!     mean_crep_tbd(i,j)=-999.99 !mean annual belowground biomass for
!     the TBD
!     PFT (kgC/m2)
!     mean_cother_tbd(i,j)=-999.99 !mean annual leaf biomass for the TBD
!     PFT
!     (kgC/m2)
...
!     mean_cbwood_herb(i,j)=-999.99 !mean annual belowground biomass for
!     the HERB PFT (kgC/m2)
!     mean_csto_herb(i,j)=-999.99 !mean annual aboveground biomass for
!     the
!     HERB PFT (kgC/m2)
!     mean_crep_herb(i,j)=-999.99 !mean annual belowground biomass for
!     the
!     HERB PFT (kgC/m2)
!     mean_cother_herb(i,j)=-999.99 !mean annual leaf biomass for the
!     HERB
!     PFT (kgC/m2)
...
!     mean_cbwood_pft(i,j,i6)=-999.99 !mean annual aboveground biomass
!     for all the PFTs (kgC/m2)
!     mean_csto_pft(i,j,i6)=-999.99 !mean annual leaf biomass for all
!     the
!     PFTs (kgC/m2)
!     mean_crep_pft(i,j,i6)=-999.99 !mean annual aboveground biomass for
!     all
!     the PFTs (kgC/m2)
!     mean_cother_pft(i,j,i6)=-999.99 !mean annual aboveground biomass
!     for
!     all the PFTs (kgC/m2)
...
!     cbwood(i,j,k) = -999.99 !average carbon content on belowground
!     wood
!     compartment in a month(KgC/m2)
!     csto(i,j,k) = -999.99 !average carbon content on storage
!     compartment in
!     a month(KgC/m2)
!     crep(i,j,k) = -999.99 !average carbon content on reproduction
!     compartment in a month(KgC/m2)
!     cother(i,j,k) = -999.99 !average carbon content on other
!     compartment in
!     a month(KgC/m2)
               do i6=1,npls
...
!     cbwood_pft(i,j,k,i6)= -999.99 !mean monthly aboveground biomass
!     for all
!     the PFTs (KgC/m2)
!     csto_pft(i,j,k,i6)= -999.99 !mean monthly leaf biomass for all the
!     PFTs
!     (KgC/m2)
!     crep_pft(i,j,k,i6)= -999.99 !mean monthly aboveground biomass for
!     all
!     the PFTs (KgC/m2)
!     cother_pft(i,j,k,i6)= -999.99 !mean monthly aboveground biomass
!     for all
!     the PFTs (KgC/m2)
...
!     cbwoodi_pft(i6) = 0.0 !inital aboveground biomass for all the PFTs
...
!     cbwoodi_tbe = cbwoodini_tbe(i,j)!inital belowground woody biomass
!     for
!     TBE PFT (KgC/m2)
...
!     cbwoodi_tbd = cbwoodini_tbd(i,j)!inital belowground woody biomass
!     for
!     TBD PFT (KgC/m2)
...
!     cbwoodi_herb = cbwoodini_herb(i,j)!inital belowground woody
!     biomass for
!     HERB PFT (KgC/m2)
...
!     cbwoodi_pft(i6) = cbwoodf_pft(i6)!it turns the carbon content of
!     the
...
!     last
!     day of the previous month on the carbon content of the 1st day of
!     the next month(for all the PFTs)
...
!     cbwoodavg_pft(i6) = 0.!mean monthly belowground biomass for all
!     the PFTs
...
c     carbon cycle (photosynthesis, plant respiration and NPP)
                     call carbon1 (temp,p0,w,wmax,ca,ipar,i6,tsoil,emax !output !input !input
     $                    ,rc2,ca2,cf2,cb2,cl1,ca1,cf1,cb1,max_index
     $                    ,ocp_pft2,beta_leaf,beta_awood,beta_bwood
...
!     carbon allocation (carbon content on each compartment)     
                     call allocation (nppa,cl1,ca1,cf1,cb1,cs1,cr1,co1 !output !input
     $                    ,i6,alc_leaf,alc_froot,alc_awood,cl2,ca2,cf2
     $                    ,cb2,cs2,cr2,co2)  

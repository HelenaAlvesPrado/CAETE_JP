...
c Variables to calculate real NPP 
	     ...
	     real ocp_tbe,ocp_tbd,ocp_herb,max_index
	     real ocp_pft2(npls)
...
c
        ...
        if (i6.ne.max_index) then
		    aux_ipar = ipar-(ipar*0.2)
		     else if (i6.eq.max_index)then
             aux_ipar= ipar 
			    endif
				  
         	 
	  jl = 0.08*(1.0-0.15)*aux_ipar*((ci-mgama)/(ci+(2.0*mgama)))
c

...
c     
c     Compile with: g95 (or gfortran) env5.f wbm4.f carbon2.f -o a.exe
c     (a.exe is the executable)
...
c     implicit none
     ...
     &     waux2(nx,ny),waux3(nx,ny),waux4(nx,ny),waux5(nx,ny),waux6(nx
     $     ,ny)
...
c     carbon allocation
     ...
     $     ,ny),cleafini_tbe(nx,ny),cawoodini_tbe(nx,ny)
     $     ,cfrootini_tbe(nx,ny),cbwoodini_tbe(nx,ny),cstoini_tbe(nx,ny)
     $     ,cotherini_tbe(nx,ny),crepini_tbe(nx,ny),cleafini_tbd(nx,ny)
     $     ,cawoodini_tbd(nx,ny),cfrootini_tbd(nx,ny),cbwoodini_tbd(nx
     $     ,ny),cstoini_tbd(nx,ny),cotherini_tbd(nx,ny),crepini_tbd(nx
     $     ,ny),cleafini_herb(nx,ny),cawoodini_herb(nx,ny)
     $     ,cfrootini_herb(nx,ny),cbwoodini_herb(nx,ny),cstoini_herb(nx
...
c
c
      var_id=0                  !define uma variavel para contabilizar cell_id
      prec_t=0.0                !define a variavel precipitacao_total
c     
c     
!     do i=1,nx
!     do j=1,ny
!     aux_des(i,j)=0 !define uma variavel para contabilizar os meses
!     com precipitacao inferior a 100mm
!     p0(i,j)=1010.0
!        enddo
!     enddo
c     
...
c     
c     !exercï½¡cio: duraõ€“­o da estaõ€“­o seca ------------------------------
...
c     
c     
c     !exercï½¡cio: mõ€•— de transiõ€“­o (estaõ€“­o seca --> chuvosa) ----------
               
               if (prec(i,j,k-1).lt.100.and.prec(i,j,k-2).lt.100
     &              .and.prec(i,j,k-3).lt.100.and.prec(i,j
     $              ,k).gt.100)then
                  trans_month(i,j)=k-1
               endif
c     
...
c     
c     !exercicio: valor de inputs para cï¼­ulas especificas -------------
               
               cellx=-60.06     !longitude
               celly=-2.35      !latitude
               long_graus=a/nx  !extensï¾†o longitudinal de cada cï¼­ula em graus
               lat_graus=b/ny   !extensï¾†o latitudinal de cada cï¼­ula em graus
...
c     if (k.eq.1.and.lat(i,j).ge.celly.and.lat(i,j).lt.(celly+0.5)!para
c     determinado mõ€•—
c     & .and.long(i,j).ge.cellx.and.long(i,j).lt.(cellx+0.5)) then
               
c     if (lat(i,j).ge.celly.and.lat(i,j).lt.(celly+0.5) !para o ano
c     inteiro
c     & .and.long(i,j).ge.cellx.and.long(i,j).lt.(cellx+0.5)) then
...
c     write cleafini_tbe (inital carbon content in the leaf compartment
c     for Tropical broadleaf evergreen PFT)
...
c     write cawoodini_tbe (inital carbon content in the aboveground wood
c     compartment for Tropical broadleaf evergreen PFT)
...
c     write cfrootini_tbe (inital carbon content in the fine roots
c     compartment for Tropical broadleaf evergreen PFT)
...
c     write cbwoodini_tbe (inital carbon content in the belowground wood
c     compartment for Tropical broadleaf evergreen PFT)
...
c     write cstoini_tbe (inital carbon content in the storage
c     compartment for Tropical broadleaf evergreen PFT)
...
c     write cotherini_tbe (inital carbon content in the other
c     compartment for Tropical broadleaf evergreen PFT)
...
c     write crepini_tbe (inital carbon content in the reproduction
c     compartment for Tropical broadleaf evergreen PFT)
...
c     write cleafini_tbd (inital carbon content in the leaf compartment
c     for Tropical broadleaf deciduous PFT)
...
c     write cawoodini_tbd (inital carbon content in the aboveground wood
c     compartment for Tropical broadleaf deciduous PFT)
...
c     write cfrootini_tbd (inital carbon content in the fine roots
c     compartment for Tropical broadleaf deciduous PFT)
...
c     write cbwoodini_tbd (inital carbon content in the belowground wood
c     compartment for Tropical broadleaf deciduous PFT)
...
c     write cstoini_tbd (inital carbon content in the storage
c     compartment for Tropical broadleaf deciduous PFT)
...
c     write cotherini_tbd (inital carbon content in the other
c     compartment for Tropical broadleaf deciduous PFT)
...
c     write crepini_tbd (inital carbon content in the reproduction
c     compartment for Tropical broadleaf deciduous PFT)
...
c     write cleafini_herb (inital carbon content in the leaf compartment
c     for Tropical Herbaceous PFT)
...
c     write cawoodini_herb (inital carbon content in the aboveground
c     wood compartment for Tropical Herbaceous PFT)
...
c     write cfrootini_herb (inital carbon content in the fine roots
c     compartment for Tropical Herbaceous PFT)
...
c     write cbwoodini_herb (inital carbon content in the belowground
c     wood compartment for Tropical Herbaceous PFT)
...
c     write cstoini_herb (inital carbon content in the storage
c     compartment for Tropical Herbaceous PFT)
...
c     write cotherini_herb (inital carbon content in the other
c     compartment for Tropical Herbaceous PFT)
...
c     write crepini_herb (inital carbon content in the reproduction
c     compartment for Tropical Herbaceous PFT)
...
c     
     ...
     c                          !exercï½¡cio: nomear as celulas de grad ----------------------------
...
c     write cawoodini (inital carbon content in the aboveground woody
c     biomass compartment)
...
c     write cfrootini (inital carbon content in the aboveground woody
c     biomass compartment)
...
c     write cbwoodini (inital carbon content in the belowground woody
c     biomass compartment)
...
c     write crepini (inital carbon content in the reproduction
c     compartment)
...
C---------------------------------------------------------------------------------
c     !Criando um arquivo_duraõ€“­o estaõ€“­o seca
...
!     real arep(3)    !npp percentage alocated to reproduction
!     compartment
...
!     crepi_aux(i,j,k) = arep(i3)*(nppot(i,j)/365)
                     ...
                     cleafi_aux(i,j,k) = ((aleaf(i3)*(nppot(i,j)))-
     &                    (cleafi_aux(i,j,k-1)/(tleaf(i3)))) +
     $                    cleafi_aux(i,j,k-1)
                     cawoodi_aux(i,j,k) = ((aawood(i3)*(nppot(i,j)))-
     &                    (cawoodi_aux(i,j,k-1)/(tawood(i3)))) +
     $                    cawoodi_aux(i,j,k-1)
                     cfrooti_aux(i,j,k) = ((afroot(i3)*(nppot(i,j)))-
     &                    (cfrooti_aux(i,j,k-1)/(tfroot(i3)))) +
     $                    cfrooti_aux(i,j,k-1)
                     cbwoodi_aux(i,j,k) = ((abwood(i3)*(nppot(i,j)))-
     &                    (cbwoodi_aux(i,j,k-1)/(tbwood(i3)))) +
     $                    cbwoodi_aux(i,j,k-1)
!     cstoi_aux(i,j,k) = ((asto(i3)*(nppot(i,j)))-
!     &		(cstoi_aux(i,j,k-1)/(tsto(i3)))) + cstoi_aux(i,j,k-1)
!     cotheri_aux(i,j,k) = ((aother(i3)*(nppot(i,j)))-
!     &		(cotheri_aux(i,j,k-1)/(tother(i3)*365))) + cotheri_aux(i
!     ,j,k-1)
!     crepi_aux(i,j,k) = ((arep(i3)*(nppot(i,j)))-
!     &		(crepi_aux(i,j,k-1)/(trep(i3)*365))) + crepi_aux(i,j,k
!     -1)
                     kk =  int(k*0.66)
...
!     &     (crepi_aux(i,j,k)/crepi_aux(i,j,kk).lt.sensitivity))   then
     ...
     $                    /cleafi_aux(i,j
     $                    ,kk).lt.sensitivity).and.(cawoodi_aux(i,j,k)
     $                    /cawoodi_aux(i,j
     $                    ,kk).lt.sensitivity2).and.(cbwoodi_aux(i,j,k)
     $                    /cbwoodi_aux(i,j,kk).lt.sensitivity2))then

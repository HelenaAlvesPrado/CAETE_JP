...
c carbon cycle
     ...
     &     monrgs(nx,ny,12), monrg(nx,ny,12)
      real mean_npp_tbe(nx,ny),mean_npp_tbd(nx,ny),mean_npp_herb(nx,ny),
     &     mean_npp_pft(nx,ny,npls), mean_ar_pft(nx,ny,npls),
     &     mean_photo_pft(nx,ny,npls),
     &	   mean_photo_tbe(nx,ny),mean_photo_tbd(nx,ny),
     &     mean_photo_herb(nx,ny),
     &     mean_ar_tbe(nx,ny),mean_ar_tbd(nx,ny),mean_ar_herb(nx,ny),
...
c carbon allocation
     ...
     & cleafini_pft(nx,ny,npls),cawoodini_pft(nx,ny,npls),
     ...
     &     ctotal_tbe(nx,ny),ctotal_tbd(nx,ny),
...
c
     ...
     &        'ctoal_pft', ctotal_pft(i,j,1), ctotal_pft(i,j,2),
     &        ctotal_pft(i,j,3) 
...
c========================================================================
      ...
      integer n
	     ...
	     data tother /0.12,0.12,0.12/
            ...
            do k=1,nt
             ...
             cleafi_aux(i,j,k) = ((aleaf(i6)*(nppot(i,j)))-
     &		(cleafi_aux(i,j,k-1)/(tleaf(i6)))) + cleafi_aux(i,j,k-1)
	        cawoodi_aux(i,j,k) = ((aawood(i6)*(nppot(i,j)))-
     &		(cawoodi_aux(i,j,k-1)/(tawood(i6)))) + cawoodi_aux(i,j,k-1)
	         cfrooti_aux(i,j,k) = ((afroot(i6)*(nppot(i,j)))-
     &		(cfrooti_aux(i,j,k-1)/(tfroot(i6)))) + cfrooti_aux(i,j,k-1)
	         cbwoodi_aux(i,j,k) = ((abwood(i6)*(nppot(i,j)))-
     &		(cbwoodi_aux(i,j,k-1)/(tbwood(i6)))) + cbwoodi_aux(i,j,k-1)
	        cstoi_aux(i,j,k) = ((asto(i6)*(nppot(i,j)))-
     &		(cstoi_aux(i,j,k-1)/(tsto(i6)))) + cstoi_aux(i,j,k-1)
	         cotheri_aux(i,j,k) = ((aother(i6)*(nppot(i,j)))-
     &		(cotheri_aux(i,j,k-1)/(tother(i6)*365))) + cotheri_aux(i,j,k-1)
	        crepi_aux(i,j,k) = ((arep(i6)*(nppot(i,j)))-
     &		(crepi_aux(i,j,k-1)/(trep(i6)*365))) + crepi_aux(i,j,k-1)
                 kk =  int(k*0.66)
     ...
     &     (cbwoodi_aux(i,j,k)/cbwoodi_aux(i,j,kk).lt.sensitivity2).and.
     ...
     &     (crepi_aux(i,j,k)/crepi_aux(i,j,kk).lt.sensitivity))   then
			   ...
			   do i1 = 20,80,40 !e.g. C-leaf
				  ...
				  alc_froot(i1,i2,i3)=i1
				  
!				  print*, alc_froot(i1,i2,i3)
				  endif
				  enddo
				  enddo
				  enddo

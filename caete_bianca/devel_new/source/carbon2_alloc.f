c234567
c=======================================================================
c     Subroutine carbon1 calculates photosynthesis, plant respiration
c     and
...
c     Rubisco, light and transport limited photosynthesis rate
c     respectively
      real jc,jl,je,
...
c      if (i6.ne.max_index) then
c         aux_ipar = ipar-(ipar*0.2)
c      else if (i6.eq.max_index)then
c         aux_ipar= ipar 
c      endif
...
!endif
      ...
      sla=(0.030*1000.)*((365/(((tleaf(pft))*365)/12))**(-0.46)) !based on Pavlick et al (2013) - (*1000) transform to m2/kg and (/30.4) transform to months
...
c     [f4 ; Eq. 18]
c     
      f4 = (1.0-(exp(-0.5*laia)))/0.5 !sun 90 deg in the whole canopy, to be used for respiration
      
c     Sun/Shade approach to canopy scaling (based in de Pury & Farquhar 1997) 
c     [f4sun, f4shade ; Eqs. 19, 20]
c     
      f4sun = (1.0-(exp(-0.5*sunlai)))/0.5 !sun 90 deg
      
      f4shade = (1.0-(exp(-1.5*shadelai)))/1.5 !sun ~20 deg
...
!     rml = (0.218*365)*(1.6**((temp-20)/10))*(ncl*cl1)
!     rmf = (0.218*365)*(1.6**((tsoil-20)/10))*(ncf*cf1)
!     rms = (0.218*365)*(1.6**((temp-20)/10))*(ncs*csa)

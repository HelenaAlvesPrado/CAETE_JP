...
c     i/o variables
      ...
      real,parameter :: NO_DATA = -9999.00
...
c     carbon allocation
     ...
     &     cbwood(nx,ny,12),csto(nx,ny,12),crep(nx,ny,12),cother(nx,ny
     $     ,12),cleafini(nx,ny),cfrootini(nx,ny),cawoodini(nx,ny)
     $     ,cbwoodini(nx,ny),cstoini(nx,ny),cotherini(nx,ny),crepini(nx
     $     ,ny),cleafini_tbe(nx,ny),cfrootini_tbe(nx,ny)
     $     ,cawoodini_tbe(nx,ny),cbwoodini_tbe(nx,ny),cstoini_tbe(nx,ny)
     $     ,cotherini_tbe(nx,ny),crepini_tbe(nx,ny),cleafini_tbd(nx,ny)
     $     ,cfrootini_tbd(nx,ny),cawoodini_tbd(nx,ny),cbwoodini_tbd(nx
     $     ,ny),cstoini_tbd(nx,ny),cotherini_tbd(nx,ny),crepini_tbd(nx
     $     ,ny),cleafini_herb(nx,ny),cfrootini_herb(nx,ny)
     $     ,cawoodini_herb(nx,ny),cbwoodini_herb(nx,ny),cstoini_herb(nx
     $     ,ny),cotherini_herb(nx,ny),crepini_herb(nx,ny),mean_cleaf(nx
     $     ,ny),mean_cawood(nx,ny),mean_cbwood(nx,ny),mean_cfroot(nx,ny)
     $     ,mean_csto(nx,ny),mean_crep(nx,ny),mean_cother(nx,ny)
     $     ,mean_cleaf_pft(nx,ny,npft),mean_cleaf_tbe(nx,ny)
     $     ,mean_cleaf_tbd(nx,ny),mean_cleaf_herb(nx,ny),cleaf_pft(nx,ny
     $     ,12,npft),cl2_pft(npft),cleafavg_pft(npft),cleafi_pft(npft)
     $     ,cleaff_pft(npft),mean_cawood_pft(nx,ny,npft)
     $     ,mean_cawood_tbe(nx,ny),mean_cawood_tbd(nx,ny)
     $     ,mean_cawood_herb(nx,ny),cawood_pft(nx,ny,12,npft)
     $     ,ca2_pft(npft),cawoodavg_pft(npft),cawoodi_pft(npft)
     $     ,cawoodf_pft(npft),mean_cbwood_pft(nx,ny,npft)
     $     ,mean_cbwood_tbe(nx,ny),mean_cbwood_tbd(nx,ny)
     $     ,mean_cbwood_herb(nx,ny),cbwood_pft(nx,ny,12,npft)
     $     ,cb2_pft(npft),cbwoodavg_pft(npft),cbwoodi_pft(npft)
     $     ,cbwoodf_pft(npft),mean_cfroot_pft(nx,ny,npft)
     $     ,mean_cfroot_tbe(nx,ny),mean_cfroot_tbd(nx,ny)
     $     ,mean_cfroot_herb(nx,ny),cfroot_pft(nx,ny,12,npft)
     $     ,cf2_pft(npft),cfrootavg_pft(npft),cfrooti_pft(npft)
     $     ,cfrootf_pft(npft),mean_csto_pft(nx,ny,npft),mean_csto_tbe(nx
     $     ,ny),mean_csto_tbd(nx,ny),mean_csto_herb(nx,ny),csto_pft(nx
     $     ,ny,12,npft),cs2_pft(npft),cstoavg_pft(npft),cstoi_pft(npft)
     $     ,cstof_pft(npft),mean_crep_pft(nx,ny,npft),mean_crep_tbe(nx
     $     ,ny),mean_crep_tbd(nx,ny),mean_crep_herb(nx,ny),crep_pft(nx
     $     ,ny,12,npft),cr2_pft(npft),crepavg_pft(npft),crepi_pft(npft)
     $     ,crepf_pft(npft),mean_cother_pft(nx,ny,npft)
     $     ,mean_cother_tbe(nx,ny),mean_cother_tbd(nx,ny)
     $     ,mean_cother_herb(nx,ny),cother_pft(nx,ny,12,npft)
     $     ,co2_pft(npft),cotheravg_pft(npft),cotheri_pft(npft)
     $     ,cotherf_pft(npft),ctotal_tbe(nx,ny),ctotal_tbd(nx,ny)
     $     ,ctotal_herb(nx,ny),ctotal_gd(nx,ny),ocp_tbe(nx,ny)
     $     ,ocp_tbd(nx,ny),ocp_herb(nx,ny)
...
c     initialize variables (-999.99 is undef)
            ...
            mphoto(i,j) =  NO_DATA
            maresp(i,j) =  NO_DATA
            meanrml(i,j) =  NO_DATA !mean annual leaf maintenance respiration (KgC/m2/yr)
            meanrmf(i,j) = NO_DATA !mean annual fine roots maintenance respiration (KgC/m2/yr)
            meanrms(i,j) = NO_DATA !mean annual sapwood maintenance respiration (KgC/m2/yr)
            meanrm(i,j) = NO_DATA !mean annual total maintenance respiration (KgC/m2/yr)
            meanrgl(i,j) = NO_DATA !mean annual leaf growth respiration (KgC/m2/yr)
            meanrgf(i,j) = NO_DATA !mean annual fine roots growth respiration (KgC/m2/yr)
            meanrgs(i,j) = NO_DATA !mean annual sapwood maintenance growth spiration (KgC/m2/yr)
            meanrg(i,j) =  NO_DATA !mean annual total growth respiration (KgC/m2/yr)
            meanhr(i,j) =  NO_DATA
            meancs(i,j) =  NO_DATA
            ...
            mean_cleaf(i,j)=  NO_DATA
            mean_cawood(i,j)=  NO_DATA
            mean_cbwood(i,j)=  NO_DATA
            mean_cfroot(i,j)=  NO_DATA
!     mean_csto(i,j)= -999.99
!     mean_crep(i,j)= -999.99
!     mean_cother(i,j)= -999.99
            mean_cleaf_tbe(i,j)= NO_DATA !mean annual leaf biomass for the TBE PFT (kgC/m2)
            mean_cawood_tbe(i,j)= NO_DATA !mean annual aboveground biomass for the TBE PFT (kgC/m2)
            mean_cbwood_tbe(i,j)= NO_DATA !mean annual belowground biomass for the TBE PFT (kgC/m2)
            mean_cfroot_tbe(i,j)=NO_DATA !mean annual leaf biomass for the TBE PFT (kgC/m2)
!     mean_csto_tbe(i,j)=NO_DATA !mean annual leaf biomass for the TBE
!     PFT (kgC/m2)
!     mean_crep_tbe(i,j)=NO_DATA !mean annual aboveground biomass for
!     the TBE PFT (kgC/m2)
!     mean_cother_tbe(i,j)=NO_DATA !mean annual belowground biomass for
!     the TBE PFT (kgC/m2)
            mean_cleaf_tbd(i,j)=NO_DATA !mean annual leaf biomass for the TBD PFT (kgC/m2)
            mean_cawood_tbd(i,j)=NO_DATA !mean annual aboveground biomass for the TBD PFT (kgC/m2)
            mean_cbwood_tbd(i,j)=NO_DATA !mean annual belowground biomass for the TBD PFT (kgC/m2)
            mean_cfroot_tbd(i,j)=NO_DATA !mean annual leaf biomass for the TBD PFT (kgC/m2)
!     mean_csto_tbd(i,j)=NO_DATA !mean annual aboveground biomass for
!     the TBD PFT (kgC/m2)
!     mean_crep_tbd(i,j)=NO_DATA !mean annual belowground biomass for
!     the TBD PFT (kgC/m2)
!     mean_cother_tbd(i,j)=NO_DATA !mean annual leaf biomass for the TBD
!     PFT (kgC/m2)
            mean_cleaf_herb(i,j)=NO_DATA !mean annual leaf biomass for the HERB PFT (kgC/m2)
            mean_cawood_herb(i,j)=NO_DATA !mean annual aboveground biomass for the HERB PFT (kgC/m2)
            mean_cbwood_herb(i,j)=NO_DATA !mean annual belowground biomass for the HERB PFT (kgC/m2)
            mean_cfroot_herb(i,j)=NO_DATA !mean annual leaf biomass for the HERB PFT (kgC/m2)
!     mean_csto_herb(i,j)=NO_DATA !mean annual aboveground biomass for
!     the HERB PFT (kgC/m2)
!     mean_crep_herb(i,j)=NO_DATA !mean annual belowground biomass for
!     the HERB PFT (kgC/m2)
!     mean_cother_herb(i,j)=NO_DATA !mean annual leaf biomass for the
!     HERB PFT (kgC/m2)
            ctotal_tbe(i,j) = NO_DATA ! it calculates the total biomass of the TBE PFT (kgC/m2)
            ctotal_tbd(i,j) = NO_DATA ! it calculates the total biomass of the TBD PFT (kgC/m2)
            ctotal_herb(i,j) = NO_DATA ! it calculates the total biomass of the HERB PFT (kgC/m2)
            mean_npp_tbe(i,j)= NO_DATA !mean annual NPP or the TBE PFT (kgC/m2/yr)
            mean_npp_tbd(i,j)= NO_DATA !mean annual NPP or the TBD PFT (kgC/m2/yr)
            mean_npp_herb(i,j)= NO_DATA !mean annual NPP or the HERB PFT (kgC/m2/yr)
            ctotal_gd(i,j)= NO_DATA !total biomass in a grid cell (the sum of all PFTs;kgC/m2/yr)
            ocp_tbe(i,j)=NO_DATA !percentage of occupation for PFT TBE
            ocp_tbd(i,j)=NO_DATA !percentage of occupation for PFT TBD
            ocp_herb(i,j)=NO_DATA !percentage of occupation for PFT HERB
            do i6=1,npft
               mean_cleaf_pft(i,j,i6)=NO_DATA !mean annual leaf biomass for all the PFTs (kgC/m2)
               mean_cawood_pft(i,j,i6)=NO_DATA !mean annual aboveground biomass for all the PFTs (kgC/m2)
               mean_cbwood_pft(i,j,i6)=NO_DATA !mean annual aboveground biomass for all the PFTs (kgC/m2)
               mean_cfroot_pft(i,j,i6)=NO_DATA !mean annual leaf biomass for all the PFTs (kgC/m2)
!     mean_csto_pft(i,j,i6)=NO_DATA !mean annual leaf biomass for all
!     the PFTs (kgC/m2)
!     mean_crep_pft(i,j,i6)=NO_DATA !mean annual aboveground biomass for
!     all the PFTs (kgC/m2)
!     mean_cother_pft(i,j,i6)=NO_DATA !mean annual aboveground biomass
!     for all the PFTs (kgC/m2)
               mean_npp_pft(i,j,i6)=NO_DATA !mean annual npp for all the PFTs (kgC/m2/yr)
            enddo
            ...
            do k=1,12
               ...
               photo(i,j,k) = NO_DATA
               aresp(i,j,k) = NO_DATA
               monrml(i,j,k) = NO_DATA !mean monthly leaf maintenance respiration (KgC/m2/day)
               monrmf(i,j,k) = NO_DATA !mean monthly fine roots maintenance respiration (KgC/m2/day)
               monrms(i,j,k) = NO_DATA !mean monthly sapwood maintenance respiration (KgC/m2/day)
               monrm(i,j,k) = NO_DATA !mean monthly total maintenance respiration (KgC/m2/day)
               monrgl(i,j,k) = NO_DATA !mean monthly leaf growth respiration (KgC/m2/day)
               monrgf(i,j,k) = NO_DATA !mean monthly fine roots growth respiration (KgC/m2/day)
               monrgs(i,j,k) = NO_DATA !mean monthly sapwood growth respiration (KgC/m2/day)
               monrg(i,j,k) = NO_DATA !mean monthly total growth respiration (KgC/m2/day)
               npp(i,j,k) = NO_DATA
               clit(i,j,k) = NO_DATA
               csoil(i,j,k) = NO_DATA
               hresp(i,j,k) = NO_DATA
               cleaf(i,j,k) = NO_DATA !average carbon content on leaf compartment in a month(KgC/m2)
               cawood(i,j,k) = NO_DATA !average carbon content on abovegroung wood compartment in a month(KgC/m2)
               cfroot(i,j,k) = NO_DATA !average carbon content on fine roots compartment in a month(KgC/m2)
               cbwood(i,j,k) = NO_DATA !average carbon content on belowground wood compartment in a month(KgC/m2)
!     csto(i,j,k) = NO_DATA !average carbon content on storage
!     compartment in a month(KgC/m2)
!     crep(i,j,k) = NO_DATA !average carbon content on reproduction
!     compartment in a month(KgC/m2)
!     cother(i,j,k) = NO_DATA !average carbon content on other
!     compartment in a month(KgC/m2)
               do i6=1,npft
                  cleaf_pft(i,j,k,i6)= NO_DATA !mean monthly leaf biomass for all the PFTs (KgC/m2)
                  cawood_pft(i,j,k,i6)= NO_DATA !mean monthly aboveground biomass for all the PFTs (KgC/m2)
                  cbwood_pft(i,j,k,i6)= NO_DATA !mean monthly aboveground biomass for all the PFTs (KgC/m2)
                  cfroot_pft(i,j,k,i6)= NO_DATA !mean monthly leaf biomass for all the PFTs (KgC/m2)
!     csto_pft(i,j,k,i6)= NO_DATA !mean monthly leaf biomass for all the
!     PFTs (KgC/m2)
!     crep_pft(i,j,k,i6)= NO_DATA !mean monthly aboveground biomass for
!     all the PFTs (KgC/m2)
!     cother_pft(i,j,k,i6)= NO_DATA !mean monthly aboveground biomass
!     for all the PFTs (KgC/m2)
                  npp_pft(i,j,k,i6)=NO_DATA !mean monthly npp for all the PFTs (KgC/m2/yr)
               enddo
            enddo
...
!     cstoi_pft(i6) = 0.0 !inital leaf biomass for all the PFTs (just
!     for initialization)
!     crepi_pft(i6) = 0.0 !inital aboveground biomass for all the PFTs
!     (just for initialization)
!     cotheri_pft(i6) = 0.0 !inital aboveground biomass for all the PFTs
!     (just for initialization)
               enddo
...
!     cstoi_tbe = cstoini_tbe(i,j)    !inital storage biomass for TBE
!     PFT (KgC/m2)
!     crepi_tbe = crepini_tbe(i,j)    !inital reproduction biomass for
!     TBE PFT (KgC/m2)
!     cotheri_tbe = cotherini_tbe(i,j)!inital other biomass for TBE PFT
!     (KgC/m2)
...
!     cstoi_tbd = cstoini_tbd(i,j)    !inital storage biomass for TBD
!     PFT (KgC/m2)
!     crepi_tbd = crepini_tbd(i,j)    !inital reproduction biomass for
!     TBD PFT (KgC/m2)
!     cotheri_tbd = cotherini_tbd(i,j)!inital other biomass for TBD PFT
!     (KgC/m2)
...
!     cstoi_herb = cstoini_herb(i,j)    !inital storage biomass for HERB
!     PFT (KgC/m2)
!     crepi_herb = crepini_herb(i,j)    !inital reproduction biomass for
!     HERB PFT (KgC/m2)
!     cotheri_herb = cotherini_herb(i,j)!inital other biomass for HERB
!     PFT (KgC/m2)
c     initialization
               do k=1,12
                  wg0(i,j,k) = -1.0
...
!     cstoi_pft(i6) = cstof_pft(i6)!it turns the carbon content of the
!     last day of the previous month on the carbon content of the 1st
!     day of the next month(for all the PFTs)
!     crepi_pft(i6) = crepf_pft(i6)!it turns the carbon content of the
!     last day of the previous month on the carbon content of the 1st
!     day of the next month(for all the PFTs) 
!     cotheri_pft(i6) = cotherf_pft(i6)!it turns the carbon content of
!     the last day of the previous month on the carbon content of the
!     1st day of the next month(for all the PFTs)       
               enddo
...
c     check if equilibrium is attained (k=12)
               if (k.eq.12) then
                  ...
                  nerro = 0
                  do kk=1,12
                     dwww = (wsoil(i,j,kk)+gsoil(i,j,kk)-wg0(i,j,kk))
     $                    /wmax
...
c     calculate tmin, meanpp, seanpp [Eqs 11, ...]
               do k=1,12
                  ...
                  mean_cawood(i,j) = mean_cawood(i,j) + (cawood(i,j,k)
     $                 /12)
                  mean_cbwood(i,j) = mean_cbwood(i,j) + (cbwood(i,j,k)
     $                 /12)
                  mean_cfroot(i,j) = mean_cfroot(i,j) + (cfroot(i,j,k)
     $                 /12)
...
c     
c     calculate PFTs total biomass and the relative PFT occupation of a
c     grid cell
...
!     &    mean_csto_herb(i,j)+ mean_crep_herb(i,j)+mean_cother_herb(i
!     ,j)
     ...
     &              mean_cbwood_tbd(i,j)+mean_cfroot_tbd(i,j)
               ...
               ctotal_herb(i,j)=mean_cleaf_herb(i,j)+mean_cawood_herb(i
     $              ,j)+mean_cbwood_herb(i,j)+mean_cfroot_herb(i,j)
               
               ctotal_gd(i,j)=ctotal_tbe(i,j)+ctotal_tbd(i,j)
     $              +ctotal_herb(i,j)
...
!     crepavg_pft(i6) = 0.!mean monthly aboveground biomass for all the
!     PFTs
!     cotheravg_pft(i6) = 0.!mean monthly belowground biomass for all
!     the PFTs
...
c     numerical integration
      do i=1,ndmonth(month)
         do i6=1,npft
            ...
            cb1 = cb2_pft(i6)
            cf1 = cf2_pft(i6)
!     cs1 = cs2_pft(i6) !transforma o valor do dia anterior no valor
!     atual
!     cr1 = cr2_pft(i6)
!     co1 = co2_pft(i6)
...
!     cr1 = crepi_tbd
               ...
               beta_froot=0.
               ocp_tbd2=ocpi_tbd
            else if ((i.eq.1).and.(month.eq.1) .and. (i6.eq.3)) then 
               cl1 = cleafi_herb
               ca1 = cawoodi_herb
               cf1 = cfrooti_herb
               cb1 = cbwoodi_herb
...
c     carbon cycle (photosynthesis, plant respiration and NPP)
            call carbon1 (temp,p0,w,wmax,ca,ipar,i6,tsoil,emax,rc2,ca2 !output !input !input
     $           ,cf2,cb2,cl1,ca1,cf1,cb1,ocp_tbe2,ocp_tbd2,ocp_herb2
     $           ,beta_leaf,beta_awood,beta_bwood,beta_froot,ph,ar,nppa
     $           ,laia,f5,rm,rml,rmf,rms,rg,rgl,rgf,rgs)
...
!     &				cs2_pft(i6)+cr2_pft(i6)+co2_pft(i6)
            ...
            ctotal_pft(i6)=cl2_pft(i6)+ca2_pft(i6)+cb2_pft(i6)
     $           +cf2_pft(i6)

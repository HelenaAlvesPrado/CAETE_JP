...
c     ------------------------- internal variables---------------------
      ...
      integer i, j, k, kk, n, mes, nerro, p, n1
      logical weq ! water equilibrium check
...
!     outputs for budget
      ...
      real BL(Q), BW(Q), BF(Q), BL0(Q), BW0(Q), BF0(Q)
         ...
         do j=1,ny
            do p = 1,q
                if (nint(lsmk(i,j)).eq.0) then
                   ...
                   cleaf_pft(i,j,p)  = 0.0 ! leaf biomass (KgC/m2)
                   cawood_pft(i,j,p) = 0.0 ! aboveground biomass (KgC/m2)
                   cfroot_pft(i,j,p) = 0.0 ! fine root biomass (KgC/m2)
                   grid_area(i,j,p)  = 0.0 ! gridcell area fraction of pfts(%)
                endif
            enddo
         enddo
      enddo
...
!     -------------------------
            if (nint(lsmk(i,j)).ne.0) then
               t0 = 0.          !Initialization
               do n1 = 1,1200   !Run to attain equilibrium
                  k = mod(n1,12)
                  if (k.eq.0) k = 12
...
!     ====================
     ...
     &             ,betafmes)
               ...
               do p=1,q
                  ...
                  betal(i,j,k,p) = betalmes(p)   ! leaf biomass change g/ha 
                  betaw(i,j,k,p) = betawmes(p)   ! awood biomass change g/ha
                  betaf(i,j,k,p) = betafmes(p)   ! froot biomass change g/ha
...
c               DO K = 1,12
c                  DO P = 1,Q
c                     BL(P) = BL(P) + BETAL(I,J,K,P)
c                     BW(P) = BW(P) + BETAW(I,J,K,P)
c                     BF(P) = BF(P) + BETAF(I,J,K,P)
c                  ENDDO
c     ENDDOcc
         

c               do p = 1,q
c                  if(abs(bl(p)) .gt. 800 .or.
c     $                abs(bf(p)) .gt. 800) then
...
c                  else
c                     continue
                     !print*, 'pft equilibrium attained', p
c                  endif   
c               enddo
            endif               ! endif lsmk
c     finalize ny loop
...
!     Accumulate daily budgets weighted by occupation coefficients
            ...
            betalavg(p) = betalavg(p) + (alfa_leaf(p) * 1e7)  
            betawavg(p) = betawavg(p) + (alfa_awood(p) * 1e7)
            betafavg(p) = betafavg(p) + (alfa_froot(p) * 1e7)

...
!     ---------
      ...
      double precision :: min_rc = 100.0D0
!     MODEL PARAMETERS
...
!     ----------------------------------------------     
      ...
      if(rc .gt. 0.001) then
         ...
         gc =  1.0/(1.0/min_rc) ! BIANCA E HELENA - Mudei este esquema..   
...
c      if(leaf_t_months .gt. 0) print*, leaf_t_months, leaf_t_coeff, pft
      ...
      sla = (3e-2 * (365.0/leaf_turnover)**(-0.46))  *(1.0 + !     ! m2/gC sla correlated with foliar N (linear)  
     &    ncl(pft)) 
...
c      sla=((0.0300*1e3)*((365./(((tleaf(pft))/365.)/12.))**(-0.46)))
      ...
      if (sla .gt. 0.0) print*, sla, laia64, cl1, 'sla' 
       laia = real(laia64,4)
      
c LAI with direct incident sun
c [sunlai; Eq. 16]      
      sunlai = (1.0-(exp(-0.5*laia64)))/0.5
c
c LAI below sunlai
c     [shadelai, Eq. 17]
      ...
      f4sun = (1.0-(exp(-0.5*sunlai)))/0.5
      f4shade = (1.0-(exp(-1.5*shadelai)))/1.5

c      if(sla .gt. 0.0) print*, laia64, f4sun, f4shade , pft!
c
c     ==============================================================================
c     !Canopy gross photosynthesis (kgC/m2/yr)
!     =======================================
!     (0.012 converts molCO2 to kgC)
!     (31557600 converts seconds to year [with 365.25 days])
      ph64 = 0.012*31557600.0*f1*f4sun*f4shade
      if(ph64 .le. 0.0 .or. isnan(ph64)) then
         ph64 = 0.0
         goto 235
      endif
      ph64 = ph64 * ocp_pft
      if(ph64 .lt. 1e-15) ph64 = 0.0
 235  continue
...
c     
c     initialization
      if(scl1 .le. 0.0005 .or. scf1 .le. 0.05) then
         scl2 = 0.0
         scf2 = 0.0
         sca2 = 0.0
         goto 10
